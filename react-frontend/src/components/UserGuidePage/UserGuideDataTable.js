...
const UserGuideDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const calendarTemplate1 = (rowData, { rowIndex }) => <p>{new Date(rowData.expiry).toLocaleDateString()}</p>
...
<Column field="expiry" header="Expiry" body={calendarTemplate1} filter={selectedFilterFields.includes("expiry")} hidden={selectedHideFields?.includes("expiry")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload UserGuide Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search UserGuide" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
