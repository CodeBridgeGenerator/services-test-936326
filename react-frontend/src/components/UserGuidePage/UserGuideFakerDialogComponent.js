...
import userGuideFakerFactory from "./userGuideFakerFactory";
...
const UserGuideFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = userGuideFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("userGuide").create(elem));
...
export default connect(mapState, mapDispatch)(UserGuideFakerDialogComponent);
