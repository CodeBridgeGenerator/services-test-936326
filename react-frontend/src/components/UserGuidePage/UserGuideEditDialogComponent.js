...
expiry: _entity?.expiry,
        ...
        const result = await client.service("userGuide").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info userGuide updated successfully" });
    ...
    return (
        <Dialog header="Edit UserGuide" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="userGuide-edit-dialog-component">
...
<div className="col-12 md:col-6 field mt-5">
        <span className="align-items-center">
            ...
            <Calendar id="expiry" value={_entity?.expiry ? new Date(_entity?.expiry) : new Date()} onChange={ (e) => setValByKey("expiry", new Date(e.target.value))} showIcon showButtonBar  inline showWeek  />
        </span>
        </div>
