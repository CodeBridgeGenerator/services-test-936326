...
const StepsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const dropdownTemplate0 = (rowData, { rowIndex }) => <p >{rowData.userGuideID?.serviceName}</p>
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.Steps}</p>
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.Description}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "steps"});
...
<Column field="userGuideID" header="UserGuideID" body={dropdownTemplate0} filter={selectedFilterFields.includes("userGuideID")} hidden={selectedHideFields?.includes("userGuideID")}  style={{ minWidth: "8rem" }} />
<Column field="Steps" header="Steps" body={pTemplate1} filter={selectedFilterFields.includes("Steps")} hidden={selectedHideFields?.includes("Steps")}  sortable style={{ minWidth: "8rem" }} />
<Column field="Description" header="Description" body={pTemplate2} filter={selectedFilterFields.includes("Description")} hidden={selectedHideFields?.includes("Description")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Steps Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Steps" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
