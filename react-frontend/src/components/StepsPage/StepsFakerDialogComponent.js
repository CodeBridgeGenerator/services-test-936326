...
import stepsFakerFactory from "./stepsFakerFactory";
...
const StepsFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [userGuideIDIds, setuserGuideIDIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("userGuide")
                        .find({ query: { $select: ["_id","serviceName"] } })
                        .then((data) => {
                        setuserGuideIDIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = stepsFakerFactory(props.user,fakerCount,userGuideIDIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("steps").create(elem));
...
export default connect(mapState, mapDispatch)(StepsFakerDialogComponent);
